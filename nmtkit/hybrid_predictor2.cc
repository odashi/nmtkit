...
HybridPredictor2::HybridPredictor2(
...
void HybridPredictor2::prepare(dynet::ComputationGraph * cg) {
  converter_.prepare(cg);
}

DE::Expression HybridPredictor2::computeLoss(
  ...
  const DE::Expression binary_loss = DE::binary_log_loss(
...
vector<Predictor::Result> HybridPredictor2::predictKBest(
    const DE::Expression & input,
...
vector<Predictor::Result> HybridPredictor2::predictByIDs(
    const DE::Expression & input,
...
NMTKIT_SERIALIZATION_IMPL(nmtkit::HybridPredictor2);
